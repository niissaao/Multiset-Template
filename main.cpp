...
void Multiset < T > ::erase(const T& value)
{
    ...
    for(ptrnow = m_head->next; ptrnow != m_head; ptrnow = ptrnow->next)
    {
        ...
        {
            ptrnow->nr--;            //just decrease the nr and size
            m_size--;
            if(ptrnow->nr == 0)          //it got to 0, delete the node
            {
...
void Multiset < T > ::eraseAll(const T& value)
{
    ...
    for(p = m_head->next; p != m_head; p = p->next)
    {
        if(p->data == value)               //interate through, find the value
        {
            //erase everything
            int m_instancesremoved = p->nr;
            Node* m_storageptr = p;
